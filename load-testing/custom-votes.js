...
async function run() {
  ...
  console.log("====================");
  console.log(
    `1 (Totally Fine):    ${"█".repeat(Math.floor((voteCounts[1] / NUM_VOTES) * 40))} ${voteCounts[1]} (${((voteCounts[1] / NUM_VOTES) * 100).toFixed(1)}%)`
  );
  console.log(
    `2 (Mostly Okay):     ${"█".repeat(Math.floor((voteCounts[2] / NUM_VOTES) * 40))} ${voteCounts[2]} (${((voteCounts[2] / NUM_VOTES) * 100).toFixed(1)}%)`
  );
  console.log(
    `3 (Either Way):      ${"█".repeat(Math.floor((voteCounts[3] / NUM_VOTES) * 40))} ${voteCounts[3]} (${((voteCounts[3] / NUM_VOTES) * 100).toFixed(1)}%)`
  );
  console.log(
    `4 (Feels Sketchy):   ${"█".repeat(Math.floor((voteCounts[4] / NUM_VOTES) * 40))} ${voteCounts[4]} (${((voteCounts[4] / NUM_VOTES) * 100).toFixed(1)}%)`
  );
  console.log(
    `5 (Crosses a Line):  ${"█".repeat(Math.floor((voteCounts[5] / NUM_VOTES) * 40))} ${voteCounts[5]} (${((voteCounts[5] / NUM_VOTES) * 100).toFixed(1)}%)`
  );
}
