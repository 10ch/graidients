  ...
  for (let i = 0; i < NUM_USERS; i++) {
    const promise = new Promise((resolve) => {
      ...
      if (rand < 0.1)
        ...
        rating = 2; // 15% - Mostly Fine
      else if (rand < 0.5)
        rating = 3; // 25% - Unsure
      else if (rand < 0.8)
        rating = 4; // 30% - Mostly Concerning
...
async function runTest() {
  ...
  try {
    ...
    console.log(
      `✅ Total Successful Votes: ${overallResults.totalSuccess}/${NUM_USERS * NUM_QUESTIONS}`
    );
    console.log(`❌ Total Errors: ${overallResults.totalErrors}`);
    console.log(
      `📈 Overall Success Rate: ${((overallResults.totalSuccess / (NUM_USERS * NUM_QUESTIONS)) * 100).toFixed(1)}%`
    );
